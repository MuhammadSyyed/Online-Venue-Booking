...
$Access = $_POST['user'];
 $conn = mysqli_connect("localhost", "root", "", "OurDB");
  ...
  $sql = "SELECT B_ID,Username,Venue,Strength,Payment,Date FROM bookings where Username = '$Access'";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["B_ID"]. "</td><td>" . $row["Username"]. "</td><td>" . $row["Venue"] . "</td><td>"
. $row["Strength"]. "</td><td>". $row["Payment"]. "</td><td>". $row["Date"]. "</td></tr>";
}
echo "</table>";
